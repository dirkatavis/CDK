...
'
' **FUNCTIONALITY:**
' Behavioral unit tests for ContainsWholeWordVtd(), IsVtdLineComplete(), and EvaluateVtdLaborGate():
...
'--------------------------------------------------------------------
' FakeBzhao — detail page + optional story page.
' SendKey "R <letter>" flips to the story page; "." flips back.
'--------------------------------------------------------------------
Class FakeBzhao
    Private m_page
    Private m_storyPage
    ...
    Public Sub SetStoryPage(ByVal pageBuf)
        m_storyPage = pageBuf
    End Sub

    Public Sub ReadScreen(ByRef outText, ByVal length, ByVal row, ByVal col)
        Dim src, pos
        If m_onStory And Len(m_storyPage) > 0 Then
            src = m_storyPage
        Else
            src = m_page
        End If
        pos = ((row - 1) * 80) + col
        If pos < 1 Then pos = 1
        outText = Mid(src, pos, length)
    End Sub

    Public Sub SendKey(ByVal keyText)
        If Left(keyText, 2) = "R " And Len(keyText) = 3 Then
            m_onStory = True
        ElseIf keyText = "." Then
            m_onStory = False
        End If
...
'--------------------------------------------------------------------
' Local copy of IsVtdLineComplete (mirrors PostFinalCharges.vbs)
'--------------------------------------------------------------------
Function IsVtdLineComplete(ByVal lineLetter)
    ...
    For r = 1 To 22
        ...
        If Err.Number <> 0 Then Err.Clear
        On Error GoTo 0
        If Len(Trim(rowBuf)) > 0 Then storyText = storyText & " " & Trim(rowBuf)
    Next
...
Function EvaluateVtdLaborGate(ByRef skipReason)
    ...
    If vtdLineLetter <> "" Then
        If IsVtdLineComplete(vtdLineLetter) Then
            ' story confirms completion — allow closeout
        Else
...
' Tests
'====================================================================
g_arrVtdCompletionKeywords = Array("COMPLETED")
...
'--------------------------------------------------------------------
' Test 7: VTD I-labor found + story screen contains "COMPLETED" -> gate PASSES
'--------------------------------------------------------------------
Set g_bzhao = New FakeBzhao
g_bzhao.SetPage BuildPage("A", "VEND TO DEALER", "I", "VTD VEND TO DEALER")
g_bzhao.SetStoryPage BuildStoryPage("LINE A STORY : 38533 SAFETY RECALL 25TA03 COMPLETED BY TOYOTA")
skipReason = ""
AssertTrue "Story: gate passes when VTD I-labor story contains COMPLETED", EvaluateVtdLaborGate(skipReason)
AssertEqual "Story: skipReason is empty on story pass", "", skipReason

'--------------------------------------------------------------------
' Test 8: VTD I-labor found + story screen has no completion keyword -> gate FAILS
'--------------------------------------------------------------------
Set g_bzhao = New FakeBzhao
g_bzhao.SetPage BuildPage("A", "VEND TO DEALER", "I", "VTD VEND TO DEALER")
g_bzhao.SetStoryPage BuildStoryPage("LINE A STORY : 38533 SAFETY RECALL 25TA03 IN PROGRESS")
skipReason = ""
AssertFalse "Story: gate fails when VTD I-labor story has no completion keyword", EvaluateVtdLaborGate(skipReason)
